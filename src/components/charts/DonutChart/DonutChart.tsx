...
import { parseData, parseLabelInput } from "../common/inputParser";

import type { EventProps } from "../common";
...
const DonutChart = React.forwardRef<HTMLDivElement, DonutChartProps>(
  (props, ref) => {
    ...
    return (
      <div ref={ref} className={cn("w-full h-40", className)} {...other}>
                      ...
                      CustomTooltip ? (
                        <CustomTooltip
                          payload={payload?.map((payloadItem) => ({
                            ...payloadItem,
                            color:
                              payload?.[0]?.payload?.color ?? BaseColors.Gray,
                          }))}
                          active={active}
